...
analyze.result.validation <- function(input, outputPlot, outputData) {
  ...
  with(pca_dta$l1, {
    ...
    text(s3d.coords$x, s3d.coords$y,     # x and y coordinates
         ...
         pos=4, cex=2)                  # shrink text 50% and place to right of points)
    # add the legend
...
# plot_data$treatment <- ml_hl_ptm$treatment
# 
# write.csv(plot_data, "/Users/davidmeng/Downloads/metprot-test/3d.csv")
# 
# p <- plot_ly(plot_data, x = ~RS1, y = ~RS2, z = ~RS3, color = plot_data$treatment, colors = c('#BF382A', '#0C4B8E')) %>%
#   add_markers() %>%
